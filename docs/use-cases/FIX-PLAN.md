...
| Mã case | Sai lệch chính | File liên quan | Mức rủi ro | Ước lượng |
|---|---|---|---|---|
| **ROLE-04** | 🔴 Staff VẪN thấy tab "Thu nhập" và VẪN gọi được `/farm/payouts` + `/farm/payouts/{id}` (nhận đủ gross/phí/net) — **chưa gate `is_owner` ở cả FE lẫn BE**. | FE `components/footer.tsx` (`FARM_NAV`), `pages/farm/payouts.tsx`; BE `FarmHubController::payouts/payoutDetail` | **Cao** | vừa |
...
| **B7** | **Phân quyền payout farm** — `FarmHubController::payouts/payoutDetail` + FE `footer.tsx`/`payouts.tsx` | **ROLE-04** | Gate `is_owner` ở BE (403 nếu staff) + ẩn tab "Thu nhập" khi `!is_owner` ở FE. |
